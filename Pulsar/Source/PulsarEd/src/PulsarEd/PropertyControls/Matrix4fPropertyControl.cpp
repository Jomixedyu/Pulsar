...
namespace pulsared
{
    bool Matrix4fPropertyControl::OnDrawImGui(const string& name, Type* type, Object* prop, std::span<Attribute*> attrs)
    {
        ...
        bool changed = false;
        changed |= ImGui::DragFloat4(("##a" + name).c_str(), a, 0.2f, 0.0f, 0.0f, "%0.3f");
        changed |= ImGui::DragFloat4(("##b" + name).c_str(), b, 0.2f, 0.0f, 0.0f, "%0.3f");
        changed |= ImGui::DragFloat4(("##c" + name).c_str(), c, 0.2f, 0.0f, 0.0f, "%0.3f");
        changed |= ImGui::DragFloat4(("##d" + name).c_str(), d, 0.2f, 0.0f, 0.0f, "%0.3f");
        if (changed)
        {
