...
namespace pulsared
{
    ...
    {
        return std::views::all(m_supportedFormats)
            | std::views::transform([](auto& v){ return string{"*"} + v; })
    ...
    AssetImporterFactory* AssetImporterFactoryManager::GetFactory(string_view name)
    {
        ...
        {
            if(factoryName == name) return factory.get();
        }
        return nullptr;
    }
    ...
    {
        return std::views::all(factories)
            | std::views::values
            | std::views::transform([](const std::unique_ptr<AssetImporterFactory>& v){ return v.get(); })
            | std::ranges::to<array_list<AssetImporterFactory*>>();
    }
} // namespace pulsared