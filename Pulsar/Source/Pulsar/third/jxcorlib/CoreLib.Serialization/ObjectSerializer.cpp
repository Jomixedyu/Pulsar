...
#define JSON_CAST(ptr) std::static_pointer_cast<VarientJson>(ptr)
    ...
    public:
        ...
        virtual void Add(const std::string& key, std::shared_ptr<Varient> value) override
        {
            js[std::string(key)] = JSON_CAST(value)->js;
        }
        ...
        {
            if (IsNull()) return VarientType::Null;
            else if (IsNumber()) return VarientType::Number;
            else if (IsString()) return VarientType::String;
            else if (IsBool()) return VarientType::Bool;
            else if (IsArray()) return VarientType::Array;
            else if (IsObject()) return VarientType::Object;
            else return VarientType::Null;
        }
        ...
        virtual std::shared_ptr<Varient> Assign(int value) override { js = value; return shared_from_this(); }

        virtual std::shared_ptr<Varient> Assign(float value) override { js = value; return shared_from_this(); }

        virtual std::shared_ptr<Varient> Assign(double value) override { js = value; return shared_from_this(); }

        virtual std::shared_ptr<Varient> Assign(bool value) override { js = value; return shared_from_this(); }

        virtual std::shared_ptr<Varient> Assign(const std::string& value) override { js = value; return shared_from_this(); }

        virtual std::shared_ptr<Varient> Assign(std::nullptr_t) override { js = nullptr; return shared_from_this(); }
