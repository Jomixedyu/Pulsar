...
#define CORELIB_REFL_DECL_FIELD(NAME, ...) \
    ...
    { \
        ...
        { \
            using RealType = decltype(std::declval<ThisClass&>().NAME); \
            using FieldType = type_redirect<RealType>::type; \
            using DecayType = fulldecay<FieldType>::type; \
            using CleanType = remove_shared_ptr<DecayType>::type; \
            using ClType = get_boxing_type<typename remove_shared_ptr<typename std::remove_cv<FieldType>::type>::type >::type; \
            using WrapType = type_wrapper<RealType>::type; \
...
namespace jxcorlib
{
    ...
    struct StaticMethodDescription final : public MethodDescription
    {
        explicit StaticMethodDescription(TReturn (*ptr)(TArgs...))
            : m_ptr(ptr)
        {
        }
    ...
    struct MemberMethodDescription final : public MethodDescription
    {
        explicit MemberMethodDescription(TReturn (TClass::*ptr)(TArgs...))
            : m_ptr(ptr)
        {
        }
    ...
    public:
        Type* GetFieldType() const { return this->m_fieldType; }
        Type* GetWrapType() const { return this->m_typeWrapper; }
    ...
    template <typename ThisClass, typename FieldClass, const char* FieldName>
    struct ReflectionFieldDeclare
    {
        using RealType = FieldClass;
        using FieldType = typename type_redirect<RealType>::type;
        using DecayType = typename fulldecay<FieldType>::type;
        using CleanType = typename remove_shared_ptr<DecayType>::type;
        using ClType = typename get_boxing_type<typename remove_shared_ptr<std::remove_cv_t<FieldType>>::type>::type;
            ...
            std::initializer_list<class Attribute*>&& arrt)
        {
            ...
            ReflectionBuilder::CreateFieldInfo<ThisClass, FieldType>(
                FieldName, true, typeWrapper, std::move(getter), std::move(setter), arrt);
        }
    };
