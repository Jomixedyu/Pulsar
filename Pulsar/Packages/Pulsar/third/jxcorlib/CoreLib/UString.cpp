...
namespace jxcorlib
{
    ...
    StringIndexMapping::StringIndexMapping(string_view str, size_t block_size)
    ...
    {
        return (mapping.get_block_count() - 1) * mapping.get_block_size() + _StringUtil_U8Length_Internal(src, mapping.mapping[mapping.get_block_count() - 1]);
    }
    ...
    std::vector<string> StringUtil::Split(string_view str, const char* splitStr)
    {
        std::vector<string> ret;
        size_t pos = 0;
        size_t offset = 0;
        const size_t step = strlen(splitStr);
        do
        {
            pos = str.find(splitStr, offset);
            ret.emplace_back(str.substr(offset, pos - offset));
            offset = pos + step;
        } while (pos != string_view::npos);
        return ret;
    }
    string StringUtil::Join(const std::vector<string>& arr, string_view str)
    {
        size_t size = 0;
    ...
    int32_t StringUtil::IndexOf(string_view name, u8char c)
    {
        int32_t pos = -1;
        ForEach(name,
                [&](u8char ch, size_t char_pos, size_t byte_pos) -> bool {
                    if (ch == c)
                    {
    ...
    void StringUtil::strcat(char* dest, size_t size, string_view source)
    {
        const auto len = ::strlen(dest);
        strcpy(dest + len, size - len, source);
    }
